...
public class WXShare {
	...
	/** 在微信开放平台注册后拿到的appid */
	private static final String APP_ID = "wxeca84b2f356693a2";
	...
	public void shareAppDataToFriend(String title, String imgPath, String description, String extInfo) {
		try {
			...
			//appdata.filePath��appdata.fileData用其一
			String path = imgPath;
			...
			msg.mediaObject = (WXMediaMessage.IMediaObject) appdata;
...
//				Toast.makeText(mContext, "抱歉，由于图片太大，分享失败！", Toast.LENGTH_SHORT).show();
				Toast.makeText(mContext, "pics too large ", Toast.LENGTH_SHORT).show();
				return;
			}
...
//					Toast.makeText(mContext, "您的微信版本不支持分享到朋友圈", Toast.LENGTH_SHORT).show();
					Toast.makeText(mContext, "wei xin banben not host", Toast.LENGTH_SHORT).show();
					return;
				}
			}	
			api.sendReq(req);
