...
public class Photo_AlbumHelper {
	...
	HashMap<String, Photo_ImageBucket> bucketList = new HashMap<>();

	private static Photo_AlbumHelper instance;

	private Photo_AlbumHelper() {
	}

	public static Photo_AlbumHelper getHelper() {
		if (instance == null) {
			instance = new Photo_AlbumHelper();
		}
		return instance;
	}
	...
	void buildImagesBucketList() {
		...
		if (cur.moveToFirst()) {
			...
			do {
				...
				Photo_ImageBucket bucket = bucketList.get(bucketId);
				if (bucket == null) {
					bucket = new Photo_ImageBucket();
					bucketList.put(bucketId, bucket);
					bucket.imageList = new ArrayList<>();
					bucket.bucketName = bucketName;
				}
				bucket.count++;
				Photo_ImageItem_Beans imageItem = new Photo_ImageItem_Beans();
		...
		Iterator<Entry<String, Photo_ImageBucket>> itr = bucketList.entrySet().iterator();
		while (itr.hasNext()) {
			Entry<String, Photo_ImageBucket> entry = (Entry<String, Photo_ImageBucket>) itr.next();
			Photo_ImageBucket bucket = entry.getValue();
			Log.d(TAG, entry.getKey() + ", " + bucket.bucketName + ", " + bucket.count + " ---------- ");
			for (int i = 0; i < bucket.imageList.size(); ++i) {
				Photo_ImageItem_Beans image = bucket.imageList.get(i);
	...
	public List<Photo_ImageBucket> getImagesBucketList(boolean refresh) {
		if (refresh || (!refresh && !hasBuildImagesBucketList)) {
			buildImagesBucketList();
		}
		List<Photo_ImageBucket> tmpList = new ArrayList<>();
		Iterator<Entry<String, Photo_ImageBucket>> itr = bucketList.entrySet().iterator();
		while (itr.hasNext()) {
			Entry<String, Photo_ImageBucket> entry = (Entry<String, Photo_ImageBucket>) itr.next();
