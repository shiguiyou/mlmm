...
public class ReplacPhoto_Look_Utils extends ImageView implements OnScaleGestureListener, OnTouchListener, ViewTreeObserver.OnGlobalLayoutListener {
     ...
     * 用于存放矩阵的20个值
     */
    private final float[] matrixValues = new float[20];
    ...
    private int widths = wm.getDefaultDisplay().getWidth();
    private int heights = wm.getDefaultDisplay().getHeight();

    public ReplacPhoto_Look_Utils(Context context) {
        this(context, null);
    }

    public ReplacPhoto_Look_Utils(Context context, AttributeSet attrs) {
        super(context, attrs);
                ...
                new SimpleOnGestureListener() {
                    ...
                    public boolean onDoubleTap(MotionEvent e) {
                        ...
                        if (getScale() < SCALE_MID) {
                            ReplacPhoto_Look_Utils.this.postDelayed(
    ...
    private class AutoScaleRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (((tmpScale > 1f) && (currentScale < mTargetScale)) || ((tmpScale < 1f) && (mTargetScale < currentScale))) {
                ReplacPhoto_Look_Utils.this.postDelayed(this, 16);
            } else {// 设置为目标的缩放比例
     ...
     * 水平方向与View的边距
     */
    private int mHorizontalPadding = (widths - 600) / 2;//——屏幕边缘离截图区的宽度
    /**
     * 垂直方向与View的边距
     */
    ...
    public Bitmap clip() {
        ...
        /*return getCircleBitmap(Bitmap.createBitmap(bitmap, mHorizontalPadding,
                mVerticalPadding, widths - 2 * mHorizontalPadding,
                widths - 2 * mHorizontalPadding));*/
