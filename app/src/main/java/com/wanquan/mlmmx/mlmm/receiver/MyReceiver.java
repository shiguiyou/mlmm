package com.wanquan.mlmmx.mlmm.receiver;
...
public class MyReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        try {
            ...
            Logger.e(TAG, "XCF1 " + intent.getAction());
            ...
            if (JPushInterface.ACTION_REGISTRATION_ID.equals(intent.getAction())) {
                ...
                SPUtils.put(mContext, "hongdianShow1", "false");
...
//                    messageEvent.setIsReceiverCode(R.id.MessageCenter_RadioButton2);
...
//                }
            ...
            if (key.equals(JPushInterface.EXTRA_NOTIFICATION_ID)) {
                ...
                try {
                    ...
                    while (it.hasNext()) {
                        ...
                        code = Integer.parseInt(json.optString(myKey));
                        if (code == 1) {
                            SPUtils.put(mContext, "hongdianShow1", "true");
                        } else if (code == 2) {
                            SPUtils.put(mContext, "hongdianShow1", "true");
                        } else if (code == 3) {
                            SPUtils.put(mContext, "hongdianShow2", "true");
                        }

                        Log.e("JIGUANG", String.valueOf(code));

                        Intent logoutIntent = new Intent();
                        logoutIntent.setAction("com.xcf.receiver");
                        logoutIntent.putExtra("receiver", code);
    ...
    //send msg to ReceiverMainActivity
    private void processCustomMessage(Context context, Bundle bundle) {
        if (ReceiverMainActivity.isForeground) {
            String message = bundle.getString(JPushInterface.EXTRA_MESSAGE);
            String extras = bundle.getString(JPushInterface.EXTRA_EXTRA);
            Intent msgIntent = new Intent(ReceiverMainActivity.MESSAGE_RECEIVED_ACTION);
            msgIntent.putExtra(ReceiverMainActivity.KEY_MESSAGE, message);
            if (!ExampleUtil.isEmpty(extras)) {
                try {
                    JSONObject extraJson = new JSONObject(extras);
                    if (extraJson.length() > 0) {
                        msgIntent.putExtra(ReceiverMainActivity.KEY_EXTRAS, extras);
                    }
                } catch (JSONException e) {
